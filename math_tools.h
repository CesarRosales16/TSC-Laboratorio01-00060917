...
void inverse(Matrix A, Matrix &inverse_matrix) {
    float det = determinant(A);
    if(det==0) exit(EXIT_FAILURE);

    Matrix cofactors_matrix, adjoint_matrix;
    cofactors(A,cofactors_matrix);
    transpose(cofactors_matrix,adjoint_matrix);

    productRealMatrix((1.0/det),adjoint_matrix,inverse_matrix);
}